...
for (i in 1:length(sorted_time_files))
{
  ...
  {
    df[i, "time_sec"] = substr(time, nchar(time) - 1, nchar(time))
    df[i, "time_min"] = str_match(
      time,
      regex(
        '[[:digit:]]*(?<=:)[[:digit:]]*(?=:)',
        ignore.case = TRUE,
    ...
    if (dim(df)[1] == 0) {
      ...
      if (dim(df)[1] == 0) {
        ...
        p_time <- p_time +
          ...
          xlab("Physlr Step") +
          scale_color_manual(
            values=c("#008000", "#8E44AD", "#2E86C1", "#808080"))
