...
params:
  path:
    ...
    value: "/projects/btl/jowong/github/physlr/data_hg004_stlfr/v2release"
    input: text
---
...
list.of.packages <- c("ggplot2", "dplyr", "stringr")
...
for (i in 1:length(sorted_time_files))
{
  ...
  exit_status <- substr(lines_of_file[exit_line_no],
                        nchar(lines_of_file[exit_line_no]), nchar(lines_of_file[exit_line_no]))
  if (exit_status != "0")
  {
  ...
  time = str_match(time_line, regex('(?<=or m:ss[[:punct:]][[:punct:]]\ ).*',
                                    ignore.case=TRUE, perl=TRUE))[1,1]
  ...
  {
    df[i,"time_sec"] = strtoi(substr(time, nchar(time)-1, nchar(time)), base=10L)
    df[i,"time_min"] = strtoi(str_match(time, regex('[[:digit:]]*(?<=:)[[:digit:]]*(?=:)',
                                                    ignore.case=TRUE, perl=TRUE)), base=10L)
    df[i,"time_hr"] = strtoi(str_match(time, regex('[[:digit:]]*(?=:)',
                                                   ignore.case=TRUE, perl=TRUE)), base=10L)

  }else if (dim(colon_marks)[1] == 1)
  {
    sec = str_match(time, regex('[[:digit:]]*(?<=:)[[:digit:]]*(?=.)',
                                ignore.case=TRUE, perl=TRUE))
    df[i,"time_sec"] = strtoi(sec, base=10L)
    df[i,"time_min"] = strtoi(str_match(time, regex('[[:digit:]]*(?=:)',
                                                    ignore.case=TRUE, perl=TRUE)), base=10L)
    df[i,"time_hr"] = 0
  }
  ...
  if (! is.na(py_pattern[1,1]))
  {
    ...
    df[i,"step"] = str_match(py_pattern, regex('(?<=\ ).*',
                                               ignore.case=TRUE, perl=TRUE))
    df[i,"pl"] = "python"
  }else
  {
    ...
    {
      df[i,"step"] = str_match(cpp_pattern, regex('(?<=-).*',
                                                  ignore.case=TRUE, perl=TRUE))
      df[i,"pl"] = "c/c++"
    }else
    {
      ...
      {
        df[i,"step"] = str_match(pattern, regex('(?<=being timed:\ \").*',
                                                ignore.case=TRUE, perl=TRUE))
        df[i,"pl"] = "third-party tool"
      }else
      {
...
if(length(to_be_removed2) > 1){
  ...
  cat("Following file(s) will be skipped due to having NA values")
...
p_time <- ggplot(df, aes(x=step, y=end, colour=pl))
p_time <- p_time +
  geom_linerange(aes(ymin=df$start, ymax=df$end), size=7) +
  coord_flip() +
  theme(text = element_text(size=12)) +
  labs(title = "Physlr - time Gantt chart",
       subtitle = paste("path:",ldir ),
       caption = "physlr version: 0.2.0")+
  theme(
    plot.title = element_text(color = "black", face = "bold"),
    plot.subtitle = element_text(color = "darkblue", size = 8),
    plot.caption = element_text(color = "darkgreen", face = "bold", size = 8)
  )+
  theme(legend.title=element_blank())+
  geom_text(label=sprintf(paste(df$time_hr, ":", df$time_min,":",df$time_sec, sep="")),
            size=2.7, hjust=-0.1, vjust=-0.1, check_overlap = T) +
  ylim(0,df$end[dim(df)[1]]*1.1) +
  ylab("Time in minutes") +
  xlab("Physlr Step")

p_mem <- ggplot(df, aes(x=step, y=mem, colour=pl))
p_mem <- p_mem +
  geom_linerange(aes(ymin=0, ymax=df$mem), size=7) +
  coord_flip() +
  theme(text = element_text(size=12)) +
  labs(title = "Physlr - memory usage",
       subtitle = paste("path:",ldir ),
       caption = "physlr version: 0.2.0")+
  theme(
    plot.title = element_text(color = "black", face = "bold"),
  ...
  theme(legend.title=element_blank())+
  geom_text(label=sprintf("%1.0f GB",df$mem),
            size=2.7, hjust=-0.1, vjust=-0.1, check_overlap = T) +
